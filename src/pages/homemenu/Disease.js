...
import Diseaseli from '../homelist/Diseaseli';
...
class Allpet extends React.Component {
    ...
    getData = async () => {
        let { message } = await fetchget('/api/getdisease/' + this.state.tid)
        this.timer = setTimeout(() => {
            this.setState(state => ({
                dsslist: [...state.dsslist, ...message],
                refreshing: false
            }))
        }, 1000);
    }
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <Diseaseli dsslist={this.state.dsslist} getData={this.getData}></Diseaseli>
            </React.Fragment>
        )
    }
}
